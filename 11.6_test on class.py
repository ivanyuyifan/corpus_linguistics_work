...
# 将结果保存到 Excel 文件
df.to_excel("corpus_eat_word_analysis.xlsx", index=False, sheet_name="Eat Word Analysis")
print("分析结果已保存到 corpus_eat_word_analysis.xlsx 文件中。")
...
print(p)
if p < 0.05:
    print("显著性检验结果：词性与'吃'字位置之间存在显著关系。")
else:
    print("显著性检验结果：词性与'吃'字位置之间不存在显著关系。")